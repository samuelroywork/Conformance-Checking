    ...
    for variant in variants:
        ...
        for net in subnets:
            if len(projectedVariant[net]) > 0:
                result[net].append(align.apply_from_variant(projectedVariant[net], net[0], net[1], net[2], parameters[net]))
            else:
                ...
                utils.add_arc_from_to(mapping[a.source.name], mapping[a.target.name], net)
                    ...
                    for trans in net.transitions:
                        if isinstance(a.source, PetriNet.Transition):
                            if(trans.label == a.source.label):
                                utils.add_arc_from_to(trans, mapping[a.target.name], net)
                                break
                        else:
                            if(trans.label == a.target.label):
                                utils.add_arc_from_to(trans, mapping[a.target.name], net)
                                break
...
# alignments = PrefromAlignment(['a,b,c,d,e','d,e,a,b,g'], subnets, parameters)
# print(alignments)
# parameters = GetAlignmentParameters(subnets)
