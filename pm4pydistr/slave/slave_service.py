...
@SlaveSocketListener.app.route("/calculateDecompAlignments", methods=["POST"])
def calculate_decomp_alignments():
    from pm4pydistr.slave import slave
    print("check4: a slave received the post request from the master")
    ...
    if keyphrase == configuration.KEYPHRASE:
        subnets_strings = content["subnets_strings"]
        var_list = content["var_list"]

        return jsonify({"alignments": slave.calculate_decomp_alignments(subnets_strings, var_list)})
