...
@MasterSocketListener.app.route("/calculateDecompAlignments", methods=["POST"])
def calculate_decomp_alignments():
    import time
    total_time_start = time.perf_counter()
    print("check1: the master recieved a post request from the user")
    check_master_initialized()
    except_if_not_slave_loading_requested()
    wait_till_slave_load_requested()
    ...
    if keyphrase == configuration.KEYPHRASE:
        alignments = MasterVariableContainer.master.calculate_decomp_alignments(session, process, use_transition, no_samples,
                                                                        petri_string,
                                                                        var_list)
        total_time = time.perf_counter()-total_time_start
        cost = alignments["alignment_cost"]
        fitness = alignments["fitness"]
        decompose_time = alignments["decompose_time"]
        recompose_time = alignments["recompose_time"]
        alignmentsres= alignments["alignment_result"]
        return jsonify({"fitness": f"{fitness:.5f}", "alignment_result": alignmentsres,"alignment_cost": cost, "total_time": f"{total_time:.5f}", "decompose_time": f"{decompose_time:.5f}", "recompose_time": f"{recompose_time:.5f}"})
    return jsonify({})
...
def perform_alignments():
    ...
    if keyphrase == configuration.KEYPHRASE:
        ...
        fitness = alignments["fitness"]
        alignmentsres= alignments["alignment_result"]
        return jsonify({"fitness": f"{fitness:.5f}", "alignment_result": alignmentsres,"alignment_cost": cost})
    return jsonify({})
